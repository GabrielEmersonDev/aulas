...
// um ano como parâmetro e retorna o título do livro publicado naquele ano e seu ano(treinar objeto). Se nenhum livro for
...
function encontrarLivroPorAno(livro, anoProcurado) {
  ...
  for (let i = 0; i < livro.length; i++) {
    if (anoProcurado === livro[i].ano) {
      livrosEncontrados.push({ titulo: livro[i].titulo, ano: livro[i].ano });
    }
  }
